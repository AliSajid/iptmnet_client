...
# Use the 1.75 version of the Rust image since it's the MSRV (Minimum Supported Rust Version) for the iptmnet_client project
...
RUN USER=root cargo new --lib iptmnet_client

# Change the working directory to the iptmnet_client directory
WORKDIR /usr/src/iptmnet_client

# Create the appropriate directory structure for the first build
RUN mkdir -p src/iptmlib && mv -v src/lib.rs src/iptmlib/lib.rs
...
RUN rm -rfv target/release/deps/iptmlib*
...
COPY --from=builder /usr/src/iptmnet_client/target/release/iptmnet /usr/local/bin/iptmnet
